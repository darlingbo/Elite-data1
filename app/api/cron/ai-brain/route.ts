...
// ── Collect live system state — READ ONLY, never modifies anything ────────────
async function collectState() {
  ...
  const [failedRes, stuckRes, pendingAgentsRes, todayRes, inventorCheck] = await Promise.all([
    supabase.from("orders").select("reference, phone, network, bundle_size").eq("status", "failed"),
    supabase.from("orders").select("id").in("status", ["pending", "processing"]).lt("created_at", cutoff15),
    supabase.from("agents").select("id, name, phone").eq("status", "pending"),
    supabase.from("orders").select("status, amount, admin_commission").gte("created_at", todayStart),
...
// AI Brain only sends alerts — it NEVER touches orders directly
type AIAction = "alert_failed" | "alert_stuck" | "alert_agents" | "alert_inventor_down" | "daily_report";

// ── Decide what to alert about ────────────────────────────────────────────────
async function decideActions(state: State): Promise<AIAction[]> {
  ...
  if (!DEEPSEEK_KEY) {
    ...
    if (state.failedOrders.length > 0) actions.push("alert_failed");
  ...
  const prompt = `You are the monitoring AI for Elite Data Ghana. You ONLY send alerts — you never take action yourself. The admin decides what to do.

Current time (Ghana): ${ghTime}
System state:
- Failed orders: ${state.failedOrders.length}
- Stuck orders (>15 min, still processing): ${state.stuckOrderCount}
- New agent applications: ${state.pendingAgents.length}
- Inventor API healthy: ${state.inventorHealthy}
...
Choose which alerts to send. Return ONLY a JSON array:
- "alert_failed" — tell admin about failed orders
- "alert_stuck" — tell admin about stuck orders
- "alert_agents" — tell admin about new agent applications
- "alert_inventor_down" — tell admin Inventor API is down
- "daily_report" — morning summary (only 07:00–07:15 Ghana time)

Return ONLY the JSON array. Example: ["alert_failed","alert_agents"]
If nothing to report: []`;
  ...
  try {
    ...
    // DeepSeek failed — safe fallback: only alert, never act
    const fallback: AIAction[] = [];
    if (state.failedOrders.length > 0) fallback.push("alert_failed");
    if (!state.inventorHealthy) fallback.push("alert_inventor_down");
    return fallback;
  }
}

// ── Main cron handler — alerts only, zero DB writes ───────────────────────────
export async function GET(request: NextRequest) {
  ...
  if (actions.length === 0) {
    return Response.json({ status: "idle", message: "All clear — nothing to report" });
  }

  const alertParts: string[] = [];

  if (actions.includes("alert_failed") && state.failedOrders.length > 0) {
    const lines = state.failedOrders.slice(0, 8)
      .map((o) => `• ${(o.network ?? "").toUpperCase()} ${o.bundle_size} → ${o.phone}`)
      .join("\n");
    const extra = state.failedOrders.length > 8 ? `\n...and ${state.failedOrders.length - 8} more` : "";
    alertParts.push(
      `❌ <b>${state.failedOrders.length} failed order(s):</b>\n${lines}${extra}\n\nSend /fix to retry them, or /failed to review one by one.`
    );
  }

  if (actions.includes("alert_stuck") && state.stuckOrderCount > 0) {
    alertParts.push(
      `⏳ <b>${state.stuckOrderCount} order(s) stuck >15 min</b>\nSend /pending to review. Use /sync to check with Inventor.`
    );
  }
  ...
  if (actions.includes("alert_inventor_down")) {
    alertParts.push(
      ...
      `Data delivery is blocked. New orders will fail.\n` +
      `Check your Inventor dashboard immediately.`
    );
  }
  ...
  if (actions.includes("daily_report")) {
    alertParts.push(
      ...
      `${state.today.failed > 0 ? `⚠️ ${state.today.failed} failed — send /failed to review` : "✅ No failures — running clean!"}`
    );
  }
  ...
  return Response.json({ status: "ok", actions });
}
