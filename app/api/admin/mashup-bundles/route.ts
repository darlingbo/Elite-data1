...
export async function POST(req: NextRequest) {
  ...
  const { name, data_value, data_unit = "GB", minutes = 0, price, cost_price = 0 } = body;
  if (!name || !data_value || !price) return Response.json({ error: "name, data_value, price required" }, { status: 400 });
  const { data, error } = await supabase.from("mashup_bundles").insert({ name, data_value, data_unit, minutes, price, cost_price, active: true }).select().single();
  if (error) return Response.json({ error: error.message }, { status: 500 });
